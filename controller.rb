...
class ShowDiffController < Controller
    ...
    def initialize(site, isAdmin, difflink, htmldiff, rcid, page, user, summary, numdiffs, log, newpage)
        @difflink = difflink
        @htmldiff = htmldiff
        @rcid     = rcid
        @page     = page
...
# Controller for the ShowLog snippet
...
# Controller for the editWL action
class EditWLController < Controller
    def initialize(wl)
        @wl = wl 
        @template = TemplateEditWL
    end
end
